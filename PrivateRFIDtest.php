...
function RFIDREAD($rfid) {
  ...
  $sql = "SELECT RFID FROM users WHERE RFID='{$rfid}' AND Status=2 ";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();

  if (!empty($row['RFID'])) {
    if ($row['RFID'] == $rfid){
    header("Location: public.php?rfid=".$row['RFID']."");
  } 
  } else {
...
<body>
  <center> <h1 style= "color:Tomato;">Please tap ID on scanner</h1> </center>
  <div class="row"></div>
  <div class="row"></div>
