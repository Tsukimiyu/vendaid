...
function RFIDREAD($rfid) {
  ...
  $sql = "SELECT Status FROM users WHERE RFID='{$rfid}' ";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();

  if (!empty($row['Status'])) {
    if ($row['Status'] == 1){
    header("Location: updateuser.php?rfid=".$rfid."");
  } 
...
<body>
  <center> <h1 style= "color:black; font-weight: medium;">Please tap ID on scanner</h1> </center>
  <div class="row"></div>
  <div class="row"></div>
